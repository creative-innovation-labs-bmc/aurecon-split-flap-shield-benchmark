...
let renderer=null,events=[],schedulerRAF=0,startPerf=0,benchmarkStart=0,syntheticMs=0;
...
function launch(date){transitionGrid(S.makeGrid(date),300,true)}
function renderTime(date,duration=220){const next=S.makeGrid(date);for(let r=1;r<=5;r++)for(let c=B.CENTRE_START;c<B.CENTRE_START+B.CENTRE_COLS;c++)renderer.setCell(r,c,next[r][c],{duration});renderer.pulseColons&&renderer.pulseColons()}
...
function startLive(){startPerf=performance.now();S.initOffices();launch(new Date());const tick=()=>{renderTime(new Date(),220);setTimeout(tick,1000-(Date.now()%1000)+8)};setTimeout(tick,8000);setInterval(()=>{S.nextOffices();renderOffices(new Date(),300)},14000)}
